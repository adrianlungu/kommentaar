...
package docparse // import "github.com/adrianlungu/kommentaar/docparse"
...
var (
	...
	reResponseHeader = regexp.MustCompile(`^Response( (\d+?))?( \((.+?)\))?: ([^\s]+)?\s?(.*)?`)
)
...
func ParseResponse(prog *Program, filePath, line string) (int, *Response, error) {
	...
	if resp[6] != "" {
		r.Body.Description = fmt.Sprintf("%s - %s", r.Body.Description, resp[6])
	}
